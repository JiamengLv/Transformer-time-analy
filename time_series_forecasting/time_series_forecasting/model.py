...
class TimeSeriesForcasting(pl.LightningModule):
    ...
    def encode_src(self, src):
        ...
        src = src_start + pos_encoder                 # 增加位置信息

        src = self.encoder(src) + src_start           # 相当于残差连接
    ...
    def decode_trg(self, trg, memory):
        ...
        trg = pos_decoder + trg_start                                                   # 位置编码

        trg_mask = gen_trg_mask(out_sequence_len, trg.device)                           # mask

        out = self.decoder(tgt=trg, memory=memory, tgt_mask=trg_mask) + trg_start       # 残差链接
...
if __name__ == "__main__":
    ...
    source = torch.rand(size=(32, 16, 9))           # source: 原始数据的输入，（batch，seq_len，emb_len)
    target_in = torch.rand(size=(32, 16, 8))        # target_in: 网络数据的输入，（batch，seq_len，emb_len)
    ...
    # a = (source, target_in)
    #
    # torch.onnx.export(ts,  target_in, onnx_path)
    #
